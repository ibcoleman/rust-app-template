...
2. **The script edits every tracked file** that contains `superbox` or `superbox`. It enumerates via `git ls-files | xargs grep -l ...` so new file types (added in later phases or by you) are covered automatically — no hardcoded list to drift.
